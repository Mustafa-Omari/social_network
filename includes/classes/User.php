...
class User
{
    ...
    // to show the photo in requests page 
    public function getProfilePicRequest($user_from) { 
        $query = mysqli_query($this->conn, "SELECT profile_pic FROM users WHERE username='$user_from'");
        $row = mysqli_fetch_array($query);
        return $row['profile_pic'];
    }
