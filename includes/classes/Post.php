<?php 
    ...
    { 
        ...
        public function submitPost($body , $user_to ,$imageName)
        {
            ...
            $body = mysqli_real_escape_string($this->conn , $body); // Remove ' ka2nu 
            $check_empty = preg_replace('/\s+/' , '' , $body); // Deletes All spaces
            
            if($check_empty != ""){ 

                // Start Insert Youtube post 
                ...
                foreach($body_array as $key => $value){
                    ...
                    if(strpos($value, "www.youtube.com/watch?v=") !== false){
                        ...
                        // https://www.youtube.com/watch?v=JmDFaxb6reo&list=RDCLAK5uy_nhydE9o1TmG1a4DdSCyOGAPBu7CNLAWis&index=1
                        // https://www.youtube.com/watch?v=JmDFaxb6reo
                        // &list=RDCLAK5uy_nhydE9o1TmG1a4DdSCyOGAPBu7CNLAWis
                        // &index=1
                        
                        $list = preg_split("!&!" , $value); // List play 
                        $value = preg_replace("!watch\?v=!", "embed/", $list[0]);
                ...
                $sql =  "INSERT INTO posts 
                            VALUES( '', '$body', '$added_by', '$user_to', '$date_added', 'no', 'no', '0', '$imageName', 'no')";
                ...
                $no_punctuation = preg_replace("/[^a-zA-Z 0-9]+/", "", $body); // (^ => not) // to replace the  punctuations to "" 
        ...
        public function calculateTrend($term) { 
            if($term != ''){
                ...
                if(mysqli_num_rows($query) == 0)
                    $insert_query = mysqli_query($this->conn, "INSERT INTO trends(title, hints) VALUES('$term', '1')");
                else
                    $insert_query = mysqli_query($this->conn, "UPDATE trends SET hints=hints+1 WHERE title='$term'");
            }
        }
        ...
        public function loadPostsFriends($data , $limit)
        {
            ...
            $data_query = mysqli_query($this->conn , "SELECT * FROM posts WHERE deleted='no' AND ((added_by='$userLoggedIn' AND privacy='yes') OR privacy='no' ) ORDER BY id DESC");
            ...
            if(mysqli_num_rows($data_query) > 0)
            {
                ...
                $count = 1 ; // Count how many result we have loaded 
                ...
                while($row = mysqli_fetch_array($data_query)){
                    ...
                    if($user_logged_obj->isFriend($added_by))
                    {
                        ...
                        if($privacy == 'no'  && $userLoggedIn == $added_by) { 
                            $privacy_button = "<button class='privacy_button btn-warning' id='privacyOnlyMe$id'>  <i class='fa fa-lock fa-lg'></i> </button>";
                        }
                        elseif($privacy == 'yes' && $userLoggedIn == $added_by) {
                            $privacy_button = "<button class='privacy_button btn-info' id='privacyPublic$id'>  <i class='fa fa-globe fa-lg'></i> </button>";
                        }
                        else {
                            $privacy_button = "";
                        }
                        ...
                        if($privacy == 'no'){
                            $privacy_message = "Public";
                        }
                        else { 
                            $privacy_message = "Only me";
                        }
                        ...
                        if($interval->y >= 1){
                            ...
                            if($interval->m == 1){
                                ...
                                $time_message = $interval->m . " monts ago";
                            }
                        ...
                        $str .= "<div class='status_post' onClick='javascript:toggle$id()'>
                                    ...
                                    <div class='posted_by' style='color:#ACACAC;'>
                                        <a href='$added_by'> $first_name $last_name </a> $user_to &nbsp;&nbsp;&nbsp;&nbsp;$time_message &nbsp;&nbsp;&nbsp;&nbsp;$privacy_message
                                        $delete_button
                                        $privacy_button
                                    </div>
                    ...
                    <script>
                        ...
                        $(document).ready(function() { 
                            ...
                            // Make Privacy to yes so just me can see this post  
                            $('#privacyOnlyMe<?php echo $id; ?>').on('click', function() {

                                bootbox.confirm("Sure, just you can see this post?", function(result) {

                                $.post("includes/privacy_handlers/privacy_only_me.php?post_id=<?php echo $id; ?>", {result:result});
                            ...
                            // Make Privacy to yes so any one can see this post 
                            $('#privacyPublic<?php echo $id; ?>').on('click', function() {

                                bootbox.confirm("Sure, anyone can see this post?", function(result) {

                                    $.post("includes/privacy_handlers/privacy_public.php?post_id=<?php echo $id; ?>", {result:result});
        ...
        public function loadProfilePosts($data , $limit)
        {
            ...
            // Only The Owner of the post can see post  if you make privacy  
            ...
            // }
            // else { 
            // $data_query = mysqli_query($this->conn , "SELECT * FROM posts WHERE deleted='no' AND privacy='no' AND ((added_by='$profileUser' AND user_to='none') OR user_to='$profileUser') ORDER BY id DESC");

            // }
            
            $data_query = mysqli_query($this->conn , "SELECT * FROM posts WHERE deleted='no' AND ((added_by='$userLoggedIn' AND privacy='yes') OR privacy='no' ) AND ((added_by='$profileUser' AND user_to='none') OR user_to='$profileUser') ORDER BY id DESC");
            ...
            if(mysqli_num_rows($data_query) > 0)
            {
                ...
                while($row = mysqli_fetch_array($data_query)){
                    ...
                    // Privacy Button ,To onlyme can see the post 
                    if($userLoggedIn == $added_by && $privacy == 'no' ){
                        $privacy_button = "<button class='privacy_button btn-warning' id='privacyOnlyMe$id'>  <i class='fa fa-lock fa-lg'></i> </button>";
                    }
                    elseif($userLoggedIn == $added_by && $privacy == 'yes') {
                        $privacy_button = "<button class='privacy_button btn-info' id='privacyPublic$id'>  <i class='fa fa-globe fa-lg'></i> </button>";
                    }
                    else { 
                        $privacy_button = "";
                    ...
                    // The Message to show the privacy status 
                    if($privacy == 'no'){
                        $privacy_message = "Public";
                    }
                    else { 
                    ...
                    $str .= "<div class='status_post' onClick='javascript:toggle$id()'>
                                ...
                                <div class='posted_by' style='color:#ACACAC;'>
                                    <a href='$added_by'> $first_name $last_name </a> &nbsp;&nbsp;&nbsp;&nbsp;$time_message &nbsp;&nbsp;&nbsp;&nbsp;$privacy_message 
                                    $delete_button
                                    $privacy_button
                                </div>
        ...
        public function getSinglePost($post_id)
        {
            ...
            if(mysqli_num_rows($data_query) > 0)
            {
                ...
                if($user_logged_obj->isFriend($added_by))
                {
                    ...
                    <script>
                        $(document).ready(function() { 
                            ...
                            $('#privacyOnlyMe<?php echo $id; ?>').on('click', function() {
                                ...
                                bootbox.confirm("Sure, just you can see the post!", function(result) {
                            ...
                            $('#privacyPublic<?php echo $id; ?>').on('click', function() {
                                ...
                                bootbox.confirm("Sure, anyone can see the post!", function(result) {
                    ...
                    echo "<p>You cannt see this post because you are not frined with this user.</p>";
                    return;
                }
